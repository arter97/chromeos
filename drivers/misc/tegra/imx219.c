...
imx219_set_mode(struct imx219_info *info, struct imx219_mode *mode)
{
	...
	offset += imx219_get_coarse_time_regs(reg_list + offset, mode->coarse_time);
...
imx219_set_group_hold(struct imx219_info *info, struct imx219_ae *ae)
{
	...
	if (ae->coarse_time_enable) {
		offset += imx219_get_coarse_time_regs(reg_list + offset, ae->coarse_time);
	}
	if (ae->gain_enable) {
...
imx219_get_af_data(struct imx219_info *info)
{
	...
	for (i = 0; i < ARRAY_SIZE(info->afdat); i++) {
		ret |= imx219_read_reg(info->i2c_client, 0x3204 + i, &info->afdat[3-i]);
	}
			...
			const struct i2c_device_id *id)
{
	...
	if (np)
	{
		...
		err = gpio_request(pw->reset_gpio, "cam-reset");
		if (err) {
			dev_err(&client->dev,
		...
		if (err) {
			dev_err(&client->dev,
			"[IMX219]:%s:Unable to set cam-reset gpio\n", __func__);
			return err;
		}

		pw->af_gpio = of_get_named_gpio(np, "af-gpios", 0);
		err = gpio_request(pw->af_gpio, "cam-af");
		if (err) {
			dev_err(&client->dev,
			"[IMX219]:%s:Unable to request af-gpio\n", __func__);
			return err;
		}
		err = gpio_direction_output(pw->af_gpio, 0);
		if (err) {
			dev_err(&client->dev,
			"[IMX219]:%s:Unable to set af-gpio\n", __func__);
			return err;
		}
	}
	else

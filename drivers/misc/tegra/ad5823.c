...
static struct ad5823_platform_data *ad5823_parse_dt(struct i2c_client *client)
{
	...
	of_property_read_string(np, "vdd_af", &pdata->vdd_af);
			...
			const struct i2c_device_id *id)
{
	...
	info->regulator = devm_regulator_get(&client->dev, info->pdata->vdd_af);
	if (unlikely(IS_ERR(info->regulator))) {
		dev_err(&client->dev, "%s %s ERR: %p\n",
			__func__, info->pdata->vdd_af, info->regulator);
	...
	memcpy(&info->miscdev, &ad5823_device, sizeof(ad5823_device));

...
static int imx208_power_get(struct imx208_info *info)
{
	...
	err |= imx208_regulator_get(info,
		&pw->avdd, info->pdata->regulators.avdd); /* ananlog 2.7v */
	err |= imx208_regulator_get(info,
		&pw->dvdd, info->pdata->regulators.dvdd); /* digital 1.2v */
	err |= imx208_regulator_get(info,
		&pw->iovdd, info->pdata->regulators.iovdd); /* IO 1.8v */
...
static struct imx208_platform_data *imx208_parse_dt(struct i2c_client *client)
{
	...
	of_property_read_string(np, "mclk-name", &pdata->mclk_name);
	pdata->cam2_gpio = of_get_named_gpio(np, "cam2-gpios", 0);
	ret = gpio_request(pdata->cam2_gpio, "cam2-reset");
	if (ret)
		return NULL;
	gpio_direction_output(pdata->cam2_gpio, 0);


	of_property_read_string(np, "avdd", &pdata->regulators.avdd);
	of_property_read_string(np, "dvdd", &pdata->regulators.dvdd);
	of_property_read_string(np, "iovdd", &pdata->regulators.iovdd);
			...
			const struct i2c_device_id *id)
{
	...
	if (!info->pdata) {
		dev_err(&info->i2c_client->dev,
			"[IMX208]:%s:Unable to get platform data\n", __func__);
		return -EFAULT;
	}
	...
	dev_err(&info->i2c_client->dev, "[IMX208]: end of probing sensor.\n");
	return 0;

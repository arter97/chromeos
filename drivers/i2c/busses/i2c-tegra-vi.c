 ...
 * Copyright (C) 2010 Google, Inc. Colin Cross <ccross@android.com>
 *
...
/**
 * struct tegra_vi_i2c_hw_feature : Different HW support on Tegra
...
struct tegra_vi_i2c_hw_feature {
...
struct tegra_vi_i2c_dev {
	struct device *dev;
	const struct tegra_vi_i2c_hw_feature *hw;
...
{
	return regulator_enable(i2c_dev->reg);
}
...
static inline int tegra_vi_i2c_clock_enable(struct tegra_vi_i2c_dev *i2c_dev)
{
	int ret;
...
static inline void tegra_vi_i2c_clock_disable(struct tegra_vi_i2c_dev *i2c_dev)
{
...
static int tegra_vi_i2c_init(struct tegra_vi_i2c_dev *i2c_dev)
{
	...
	err = tegra_vi_i2c_clock_enable(i2c_dev);
	if (err < 0) {
	...
	struct i2c_msg *msg, enum msg_end_type end_state)
{
	 ...
	 * the STOP condition on the bus. So wait for 2 clock periods before
	 * resetting the controller so that STOP condition has been delivered
	 * properly.
	 */
	...
	int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		ret = tegra_vi_i2c_xfer_msg(i2c_dev, &msgs[i], end_type);
		if (ret)
			break;
	}
	tegra_vi_i2c_clock_disable(i2c_dev);
...
static const struct tegra_vi_i2c_hw_feature tegra210_vi_i2c_hw = {
...
static const struct of_device_id tegra_vi_i2c_of_match[] = {
	{ .compatible = "nvidia,tegra210-vii2c", .data = &tegra210_vi_i2c_hw, },
	{},
};
MODULE_DEVICE_TABLE(of, tegra_vi_i2c_of_match);
...
static int tegra_vi_i2c_probe(struct platform_device *pdev)
{
	...
	i2c_dev->reg = devm_regulator_get(&pdev->dev, "avdd-dsi-csi");
	...
	i2c_dev->hw = &tegra210_vi_i2c_hw;

	if (pdev->dev.of_node) {
		const struct of_device_id *match;

		match = of_match_device(tegra_vi_i2c_of_match, &pdev->dev);
	...
	ret = devm_request_irq(&pdev->dev, i2c_dev->irq,
			tegra_vi_i2c_isr, 0, dev_name(&pdev->dev), i2c_dev);
	if (ret) {
...
static int tegra_vi_i2c_suspend(struct device *dev)
{
...
static int tegra_vi_i2c_resume(struct device *dev)
{
	struct tegra_vi_i2c_dev *i2c_dev = dev_get_drvdata(dev);
	int ret;
...
static SIMPLE_DEV_PM_OPS(
		tegra_vi_i2c_pm,
		tegra_vi_i2c_suspend,
		tegra_vi_i2c_resume);
#define TEGRA_VI_I2C_PM	(&tegra_vi_i2c_pm)
#else
#define TEGRA_VI_I2C_PM	NULL
#endif

static struct platform_driver tegra_vi_i2c_driver = {
	.probe   = tegra_vi_i2c_probe,
	.remove  = tegra_vi_i2c_remove,
	.driver  = {
		.name  = "tegra-vii2c",
		.owner = THIS_MODULE,
		.of_match_table = tegra_vi_i2c_of_match,
		.pm    = TEGRA_VI_I2C_PM,
	},
};

module_platform_driver(tegra_vi_i2c_driver);
